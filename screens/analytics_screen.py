...
def render_analytics_screen(parent_frame, use_mock=False):
    ...
    if not summaries["per_day"].empty:
        ...
        ax1.bar(summaries["per_day"]["date"].astype(str), summaries["per_day"]["count"],
    ...
    if not summaries["per_type"].empty:
        fig2, ax2 = plt.subplots(figsize=theme["chart_size_small"])
        ax2.pie(
    ...
    for i, (key, value) in enumerate(streaks.items()):
        tk.Label(
            ...
            relief="groove",
            padx=10, pady=5
        ).grid(row=0, column=i, padx=10, pady=5, sticky="nsew")
    ...
    # Optional button to force height recalculation (debugging only)
